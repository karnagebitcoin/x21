...
import ImageCommandList, { ImageCommandListHandle, ImageCommandListProps } from './ImageCommandList'

const suggestion = {
  items: async ({ query }: { query: string }) => {
    // Return the query as-is for the image generation
    return [{ query }]
  },

  render: () => {
    let component: ReactRenderer<ImageCommandListHandle, ImageCommandListProps> | undefined
    let popup: Instance[] = []
    ...
    return {
      ...
      onStart: (props: { editor: Editor; clientRect?: (() => DOMRect | null) | null; query?: string }) => {
        component = new ReactRenderer(ImageCommandList, {
          props: {
            ...props,
            query: props.query || ''
          },
