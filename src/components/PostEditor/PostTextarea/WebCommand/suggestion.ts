...
import WebCommandList, { WebCommandListHandle, WebCommandListProps } from './WebCommandList'

const suggestion = {
  items: async ({ query }: { query: string }) => {
    // Return the query as-is for the web search to process
    return [{ query }]
  },

  render: () => {
    let component: ReactRenderer<WebCommandListHandle, WebCommandListProps> | undefined
    let popup: Instance[] = []
    ...
    return {
      ...
      onStart: (props: { editor: Editor; clientRect?: (() => DOMRect | null) | null; query?: string }) => {
        const parentEvent = props.editor.extensionManager.extensions.find(
          (ext) => ext.name === 'webCommand'
        )?.options.parentEvent

        component = new ReactRenderer(WebCommandList, {
          props: {
            ...props,
            query: props.query || '',
            parentEvent
